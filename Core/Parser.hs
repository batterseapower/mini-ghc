...
parse :: FilePath -> IO [(Var, Term)]
parse path = do
    -- Read and pre-process .core file
    contents <- readFile path
    
    -- Return parsed .core file
    return (moduleCore . LHE.fromParseResult . LHE.parseFileContentsWithMode (LHE.defaultParseMode { LHE.parseFilename = path, LHE.extensions = [LHE.CPP, LHE.MagicHash] }) $ contents)
...
scrutinise e alts = case_ e alts
